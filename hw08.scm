...
(define (longest-increasing-subsequence lst)
  (if (null? lst) nil
    (let (
      (with (cons (car lst) (longest-increasing-subsequence (filter (lambda (with) (> with (car lst))) (cdr lst)))))
      (without (longest-increasing-subsequence (cdr lst))))
      (if (> (length with) (length without)) with without)
      )))
