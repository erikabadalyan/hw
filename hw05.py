...
def replace_leaf(t, old, new):
    ...
    if is_leaf(t):
        ...
        new_tree = [replace_leaf(b, old, new) for b in branches(t)]
...
def totals_tree(m):
    ...
    if is_weight(m):
        ...
        branches_list = [totals_tree(left_side), totals_tree(right_side)]
        return tree(total_weight(m), branches_list)
...
#store if a gets called..store that a into something store it in a dictionary with the curly brackets

    counter_dictionary = {}
    def helper(a):
        if a not in counter_dictionary:
            counter_dictionary[a] = 1
        else:
            counter_dictionary[a] += 1
        return counter_dictionary[a]
    return helper
...
def make_withdraw(balance, password):
    ...
    def withdraw(amount, passwd):
        ...
        nonlocal incorrect
        if len(incorrect) >= 3:
            return "Your account is locked. Attempts: ['" + incorrect[0] + "', '" + incorrect[1] + "', '" + incorrect[2] + "']"
        else:
            if passwd == password:
                if balance > amount:
